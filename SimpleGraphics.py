...
def MakeWindow(M,labels=True,bgcolor=WHITE,Click=False):
    """
    ...
    if Click:
        ...
        def OnClick(event):
            try:
                global Lx
                global Ly
                Lx.append(int(floor(event.xdata/2.0)))
                Ly.append(int(floor(event.ydata/2.0)))
                if len(Lx)>1:
                    if Lx[1]==Lx[0] and Ly[1]==Ly[0]:
                        Lx=[]
                        Ly=[]
                        print 'Move Cleared\n'
